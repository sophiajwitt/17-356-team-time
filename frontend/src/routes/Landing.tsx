    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-[#fce6d2]">
            <div className="bg-white p-8 rounded-2xl shadow-lg w-96 text-center flex flex-col gap-2">
                ...
                <NavLink className="w-full bg-[#faab99] text-white py-2 rounded mb-2 hover:bg-[#f89686] flex justify-center items-center"
                    to={"/register"}>
                    {"Register"}
                </NavLink>

            </div>
            { profiles.length > 0 && <div className="bg-white p-8 rounded-2xl shadow-lg w-96 text-center mt-8">
            <h2 className="text-xl font-semibold mb-4">Registered Users:</h2>
            <div className="flex flex-row flex-wrap gap-2">
                {profiles.map((p) => (
                    <NavLink to={`profile/${p.userId}`} className="bg-blue-200 text-slate-700 h-10 w-20 rounded-lg flex justify-center items-center px-2">
                        {p.userId}
                    </NavLink>
                ))}
            </div>
